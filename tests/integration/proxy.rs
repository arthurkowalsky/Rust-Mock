...
const PROXY_TARGET: &str = "https://api.restful-api.dev";
...
async fn test_endpoint_with_proxy_url() {
    ...
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body.is_array(), "Expected array response from restful-api.dev");
}
...
async fn test_default_proxy_mode() {
    ...
    assert_eq!(resp.status().as_u16(), 200);
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body.is_array(), "Expected array from proxied request");
}
...
async fn test_mixed_mock_and_proxy() {
    ...
    assert!(body2.is_array(), "Expected proxied response");
}
...
async fn test_proxy_with_query_params() {
    ...
    let resp = client
        .get(format!("{}/objects?id=1&id=2", BASE_URL))
        .send()
        .await
        .unwrap();
...
async fn test_proxy_post_with_body() {
    ...
    assert_eq!(resp.status().as_u16(), 200);
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body.get("id").is_some(), "Expected created object with id");
    assert_eq!(body["name"], "Test Object");
}
